...
def behaviorSupport_behavior_multiple_tournaments(request):
    ...
    if not data["wantedTournaments"]:
        queryResult = BehaviorSupport.objects.exclude(tournament__exact="League of Legends Scrims")
    ...
    for tournament in wantedTournaments:
        queryResult = BehaviorSupport.objects.filter(tournament__exact=tournament)
        serializer = BehaviorSupportSerializer(queryResult, context={"request": request}, many=True)
    ...
    transformed_result_scaled : pd.DataFrame = compute(df_all_player, model_uuid, tournamentDict, header_offset=8, role="Support")
