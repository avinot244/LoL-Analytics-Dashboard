...
def behaviorMid_get_player_list(request, patch, scrim):
    if scrim == 0:
        allObjects = BehaviorMid.objects.filter(patch__contains=patch).filter(~Q(tournament="League of Legends Scrims"))
    else:
        allObjects = BehaviorMid.objects.filter(patch__contains=patch, tournament__exact="League of Legends Scrims")
...
def behaviorMid_get_player_list_tournament(request, patch, tournament):
    if tournament == "League of Legends Scrims":
        allObjects = BehaviorMid.objects.filter(patch__contains=patch, tournament__exact=tournament, date__gte=DATE_LIMIT)
    else:
...
def behaviorMid_stats_tournament(request, summonnerName, tournament):
    summonnerNameList : list = list()
    if tournament == "League of Legends Scrims":
        allObjects = BehaviorMid.objects.filter(tournament__exact=tournament, date__gte=DATE_LIMIT)
    else:
...
def behaviorMid_behavior_multiple_tournaments(request):
    ...
    if not data["wantedTournaments"]:
        queryResult = BehaviorMid.objects.exclude(tournament__exact="League of Legends Scrims")
