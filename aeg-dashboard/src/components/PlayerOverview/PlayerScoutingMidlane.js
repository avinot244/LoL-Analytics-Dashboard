...
export default function PlayerScoutingMid(props){
    ...
    const fetchData = async (tournamentList) => {
        setLoadingData(true)
        const modelResult = await fetch(API_URL + `behaviorModels/getModel/Mid/`, {
            method: "GET",
            headers:header
        })
        modelResult.json().then(async model => {
            ...
            const dataResult = await fetch(API_URL + "behavior/Mid/computeScouting/", {
                method: "PATCH",
            ...
            dataResult.json().then(result => {
                ...
                for (let i = 0; i < newData.summonnerName.length; i++) {
                    temp.push({
                        ...
                        farming_safely: ((1-normDist.cdf(newData.Factor_1[i]))*100).toFixed(2),
                        aggressive_player: ((1-normDist.cdf(newData.Factor_2[i]))*100).toFixed(2),
                        roaming_bot: ((1-normDist.cdf(newData.Factor_3[i]))*100).toFixed(2),
                        tower_objective: ((1-normDist.cdf(newData.Factor_4[i]))*100).toFixed(2),
                        skirmish: ((1-normDist.cdf(newData.Factor_5[i]))*100).toFixed(2),
                        vision: ((1-normDist.cdf(newData.Factor_6[i]))*100).toFixed(2),
                        invader: ((1-normDist.cdf(newData.Factor_7[i]))*100).toFixed(2),
                        roaming_top: ((1-normDist.cdf(newData.Factor_8[i]))*100).toFixed(2),
                    })
                }
    ...
    return (
        <div className="wrapper-player-scouting-Mid">
            <div
                role='tabpanbel'
                            ...
                            </Stack>
                            {
                                flagDisplayData && (
                                    loadingData ? (
                                        ...
                                        <Paper sx={{ height: 400, width: '100%', marginMid: '15px'}}>
                                            <DataGrid
                                                rows={rows}
                                                columns={columns}
