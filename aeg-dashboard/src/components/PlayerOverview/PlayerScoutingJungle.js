...
export default function PlayerScoutingJungle(props){
    ...
    const fetchData = async (tournamentList) => {
        setLoadingData(true)
        const modelResult = await fetch(API_URL + `behaviorModels/getModel/Jungle/`, {
            method: "GET",
            headers:header
        })
        modelResult.json().then(async model => {
            ...
            const dataResult = await fetch(API_URL + "behavior/Jungle/computeScouting/", {
                method: "PATCH",
            ...
            dataResult.json().then(result => {
                ...
                for (let i = 0; i < newData.summonnerName.length; i++) {
                    temp.push({
                        ...
                        invader: ((1-normDist.cdf(newData.Factor_1[i]))*100).toFixed(2),
                        aggressive_player: ((1-normDist.cdf(newData.Factor_2[i]))*100).toFixed(2),
                        skirmish: ((1-normDist.cdf(newData.Factor_3[i]))*100).toFixed(2),
                        farming_safely: ((1-normDist.cdf(newData.Factor_4[i]))*100).toFixed(2),
                        playing_botside: ((1-normDist.cdf(newData.Factor_5[i]))*100).toFixed(2),
                        playing_topside: ((1-normDist.cdf(newData.Factor_6[i]))*100).toFixed(2),
                    })
                }
                            ...
                            </Stack>
                            {
                                flagDisplayData && (
                                    loadingData ? (
                                        ...
                                        <Paper sx={{ height: 400, width: '100%', marginJungle: '15px'}}>
                                            <DataGrid
                                                rows={rows}
                                                columns={columns}
