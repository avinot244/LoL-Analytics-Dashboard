...
export default function PlayerScoutingSupport(props){
    ...
    const fetchData = async (tournamentList) => {
        setLoadingData(true)
        const modelResult = await fetch(API_URL + `behaviorModels/getModel/Support/`, {
            method: "GET",
            headers:header
        })
        modelResult.json().then(async model => {
            ...
            const dataResult = await fetch(API_URL + "behavior/Support/computeScouting/", {
                method: "PATCH",
            ...
            dataResult.json().then(result => {
                ...
                for (let i = 0; i < newData.summonnerName.length; i++) {
                    temp.push({
                        ...
                        playing_w__jungle: ((1-normDist.cdf(newData.Factor_1[i]))*100).toFixed(2),
                        roaming_jungle: ((1-normDist.cdf(newData.Factor_2[i]))*100).toFixed(2),
                        generating_difference: ((1-normDist.cdf(newData.Factor_3[i]))*100).toFixed(2),
                        roaming_top: ((1-normDist.cdf(newData.Factor_4[i]))*100).toFixed(2),
                        fighter_dps: ((1-normDist.cdf(newData.Factor_5[i]))*100).toFixed(2),
                        roaming_mid: ((1-normDist.cdf(newData.Factor_6[i]))*100).toFixed(2),
                        vision: ((1-normDist.cdf(newData.Factor_7[i]))*100).toFixed(2),
                    })
                }
                            ...
                            </Stack>
                            {
                                flagDisplayData && (
                                    loadingData ? (
                                        ...
                                        <Paper sx={{ height: 400, width: '100%', marginSupport: '15px'}}>
                                            <DataGrid
                                                rows={rows}
                                                columns={columns}
