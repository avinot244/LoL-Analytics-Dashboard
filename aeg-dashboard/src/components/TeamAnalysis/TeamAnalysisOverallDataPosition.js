...
function TeamAnalysisOverallDataPosition({timeFrame, team, tournamentList, side}) {
    ...
    const fetchPlayerPositionOverall = async (position) => {
        const data = {
            "role": position,
            "side": side,
    ...
    useEffect(() => {
        roleList.forEach(role => {
            console.log(role)
            fetchPlayerPositionOverall(role)
        })
        setVisible(true)
    }, [])
...
export default TeamAnalysisOverallDataPosition